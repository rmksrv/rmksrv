...
{#if works?.length}
  <div class="works mb-8 md:mb-16">
    ...
    >
      My <span class={`${mainSettings.textColorScheme}`}>Works</span>
    </h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 py-3">
      {#each works as { metadata, path }, index}
        {#if index < mainSettings.numberOfWorksToShow}
